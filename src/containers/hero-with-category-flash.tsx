...
const HeroWithCategoryFlash: React.FC<Props> = ({ className = 'mb-12 md:mb-14 xl:mb-16' }) => {
    return (
        <div className={`grid grid-cols-1 gap-5 lg:grid-cols-7 lg:gap-y-14 xl:gap-7 2xl:grid-cols-9 ${className}`}>
            <CategoryListCardSection />
            <div className="col-span-full row-span-full grid grid-cols-2 gap-2 md:gap-3.5 lg:col-span-5 lg:row-auto lg:gap-5 xl:col-span-5 xl:gap-7">
    ...
    return (
        <>
            {error ? (
                ...
                <div className="col-span-full grid grid-cols-1 justify-between gap-3 lg:col-span-2">
                    {isLoading
                        ? Array.from({ length: 7 }).map((_, idx) => (
